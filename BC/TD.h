...
public:
    ...
    Shortcut merge(Shortcut const& sc);//already exist
...
public:
    ...
    vector< map<int, int> > E;//E[u][v]的值就是sc_pool中的第几个shortcut
...
public:
    ...
    vector<vector<int> > length, flength, sigma_bar, f_sigma_bar;
    ...
    int last_height;
    vector<vector<pair<int, int>>> SPPG;
    vector<double> BC;
